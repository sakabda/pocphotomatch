...
export class AppComponent {
  ...
  private readonly BATCH_SIZE = 900; // Size of each upload batch
...
//   }
 ...
 handleUpload(event: Event): void {
  ...
  private uploadBatch(files: NzUploadFile[], batchNumber: number, totalBatches: number): Observable<any> {
    const formData = new FormData();
    files.forEach((file) => {
      if (file.originFileObj) {
        formData.append('files', file.originFileObj);
    ...
    return this.http.post(`${this.apiUrl}/upload_images/`, formData, {
      reportProgress: true,
      observe: 'events',
    }).pipe(
      map(event => {
      ...
      catchError((error) => {
        this.message.error(`Batch ${batchNumber + 1} upload error`);
        return of(null);
      })
    );
  }
